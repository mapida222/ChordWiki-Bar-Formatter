...
ChordPro形式のコード譜へ、小節線と音の長さを表す記号を追加するブラウザー内ツールです。
入力内容をサーバーへ送らず、HTML、CSS、JavaScriptだけで変換します。

このプロジェクトはChordWikiの非公式ツールです。
ChordWikiの運営元とは関係ありません。

![ヘルプ画面](help-usage-screenshot.png)
...
3. 必要に応じて「03. 初期設定」を変更します。
4. 「04. 行修正」でコードごとの長さを調整します。
5. 「06. 譜面プレビュー」で表示を確認します。
6. 「07. 確定譜面テキスト」で確定し、結果をコピーします。

ローカルファイルとして開くだけで動作します。
静的Webサーバーへ配置した場合も同じ機能を利用できます。

## データの扱い

変換処理はブラウザー内で完結します。
このアプリ自身は、入力したコード譜や設定を外部サーバーへ送信しません。
...
保存内容は同じブラウザーと同じ公開元で利用されます。
ブラウザーのサイトデータを削除すると、保存内容も削除されます。
...
テストは`tests`内の全ファイルと、主要JavaScriptの構文を検査します。
外部パッケージのインストールは不要です。
