...
assert.strictEqual((html.match(/class="correction-action-divider"/g) || []).length, 3, "row-edit actions must be condensed with pipe separators");
assert(css.includes(".correction-context-bar { position: relative; display: flex;"));
...
assert(css.includes(".correction-history-actions { display: flex; flex-wrap: wrap;"));
...
assert(css.includes(".output-card { container-type: inline-size; grid-column: 2; grid-row: 2; margin-top: var(--result-controls-offset, 0px); }"));
