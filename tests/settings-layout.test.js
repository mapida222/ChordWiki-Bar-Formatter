...
assert(app.includes('<span class="setting-number">${index + 1}.</span>&nbsp;${definition.label}'), "設定番号と項目名の間に見やすい空白を入れる");
assert(app.includes('${guideMarkup}</span></span>'), "設定項目の？ヘルプも文ごとの改行済み説明を使う");
assert(app.includes('replace(/。(?![\\r\\n]|$)/gu, "。<br>").replace(/\\r?\\n/gu, "<br>")'), "設定項目の？ヘルプは指定した改行も表示する");
assert(app.includes('　前の拍に寄せる：歌詞を1文字前へ移動（デフォルト）\\n　そのまま：歌詞位置を変えない。'), "選択肢の説明は行ごとに字下げして表示する");
assert(app.includes('行修正で長い拍を指定したときの、歌詞の置き方を選びます。\\n通常：歌詞を小節内で均等に配置します。'), "長い拍の歌詞配置には選択内容を示す導入文を表示する");
assert(app.includes('例）前に寄せる：[C][----]あ[B][---]い[|]'), "端数の歌詞位置には結果例を表示する");
assert(app.includes('例）表示する：[A]----|[A]----|'), "コードなし小節の表示には結果例を表示する");
assert(app.includes('例）追記あり：[C][----]あ[----][|]'), "1文字小節のハイフンには結果例を表示する");
assert(app.includes('positionSettingHelpPopover') && app.includes('popover.style.position = "fixed"'), "初期設定のヘルプは画面前面に固定表示する");
assert(app.includes('popover.style.width = "max-content"') && app.includes('const width = Math.min(popover.scrollWidth, window.innerWidth - (margin * 2));'), "設定の？ヘルプは内容に合わせて幅を決める");
...
assert(html.includes('help-tip-settings">') && html.includes('曲の拍子に合わせて、<br>自動で補うハイフン数'), "初期設定の？ヘルプを読みやすく改行する");
assert(html.includes('行修正値を更新します。<br><b>変換後から行修正を復元</b>'), "行修正操作の？ヘルプを読みやすく改行する");
...
assert(css.includes("width: max-content;") && css.includes("max-width: calc(100vw - 16px);") && css.includes(".setting-help .context-help-popover { white-space: nowrap; }") && css.includes("overflow-wrap: normal;") && css.includes("word-break: keep-all;"), "設定の？ヘルプは幅を広く使い、文中で不自然に改行しない");
