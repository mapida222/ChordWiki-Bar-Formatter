...
const parsedReport = reportApi.parse(report);
assert.strictEqual(parsedReport.name, snapshot.title);
assert.strictEqual(parsedReport.input, snapshot.inputText);
assert.strictEqual(parsedReport.corrections, snapshot.correctionText);
assert.deepStrictEqual(reportApi.toHistorySnapshot(parsedReport).settings.converter, snapshot.settings.converter);
assert.throws(() => reportApi.parse("plain text"), /不具合報告用テキスト/);
...
assert(css.includes(".history-file-panel"));
assert(app.includes("downloadIssueReport(currentFileSnapshot())"));
assert(app.includes("downloadBackup(currentFileSnapshot())"));
assert(app.includes("importIssueReportFile(file)"));
