...
assert.strictEqual((guideMarkup.match(/<details class="guide-item">/g) || []).length, 10);
...
assert.strictEqual(guideDetails.length, 10);
...
assert(guideText.includes("/アウフタクト・小節またぎ"));
...
const pickupBar = CBFConverter.convertChordText("[G/B]--|[C]----[D]----|", base, ["2/44"]);
assert.strictEqual(pickupBar.output, "[G/B][--][|][C][----][D][----][|]");
const multiChordPickup = CBFConverter.convertChordText("[C]--[G/B]--|[A]----[Bm]----|", base, ["22/44"]);
assert.strictEqual(multiChordPickup.output, "[C][--][G/B][--][|][A][----][Bm][----][|]");
const unevenMultiChordPickup = CBFConverter.convertChordText("[G/B]--[C]-|[A]----[Bm]----|", base, ["21/44"]);
assert.strictEqual(unevenMultiChordPickup.output, "[G/B][--][C][-][|][A][----][Bm][----][|]");
assert.strictEqual(CBFConverter.renderWithBeatCode("[G/B]--[C]-|[D]----[Em]----|", "3/44", base).ok, false, "a pickup marker cannot span multiple chord slots");
