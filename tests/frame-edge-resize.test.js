...
assert(app.includes("function captureResizeScrollPositions()") && app.includes("function restoreResizeScrollPositions(positions)"), "column resizing must provide horizontal viewport preservation helpers");
assert(app.includes("let columnResizeActive = false;") && app.includes("let columnResizeScrollPositions = null;"), "column resizing must track its active viewport-preservation state");
assert(app.includes("const resizeScrollPositions = columnResizeScrollPositions || captureResizeScrollPositions();") && app.includes("restoreResizeScrollPositions(resizeScrollPositions);"), "column resizing must restore editor horizontal positions after each width update");
assert(app.includes("const rightOffset = Math.max(0, maxScrollLeft - editor.scrollLeft);") && app.includes("rightOffset <= Math.max(2, paddingRight + 2)"), "a pane already at the visual right edge must stay at its new right edge while resizing");
assert(app.includes("if (columnResizeActive) return;"), "linked horizontal scroll must not overwrite a viewport while a column is being resized");
assert(app.includes("function beginColumnResize(event)") && app.includes("function finishColumnResize()"), "all horizontal resize handles must share the viewport-preservation lifecycle");
assert((app.match(/beginColumnResize\(event\);/g) || []).length >= 2, "column edges and corner handles must capture the viewport before resizing");
