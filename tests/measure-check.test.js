...
assert(html.includes('id="committed-measure-check-meter"'), "the pre-analysis meter choice panel must exist");
...
assert(entry.includes('import "../measure-check.js?v=20260906-005"'));
assert(entry.includes('import "../committed-measure-check-panel.js?v=20260906-005"'));
...
assert(panel.includes("proposeMeterAnnotation"));
assert(panel.includes("1小節だけの一時的な拍数の乱れは、拍子変更として急かさない。"), "meter suggestions must not appear for a single temporary measure mismatch");
assert(panel.includes("同じ長さの小節が続いています。書き忘れか、意図した変化かを確認してください。"), "meter guidance must use beginner-friendly wording");
assert(panel.includes("${defaultMeter}として保管") && panel.includes("meter: defaultMeter"), "the store-as-default action must use the active initial meter");
assert(panel.includes("meterCandidateLocationKey") && panel.includes("appliedMeterDismissalLocations"), "deleting an applied meter annotation must suppress re-proposals at that measure location during the current edit session");
assert(panel.includes("手動で確認"));
assert(panel.includes("追加しない（再提案しない）"));
