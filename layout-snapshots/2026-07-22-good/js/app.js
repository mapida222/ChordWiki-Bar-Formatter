(function () {
  ...
  const elements = {
    ...
    removalTargets: $("#hyphen-removal-targets"), removalLinked: $("#hyphen-removal-linked"), removalSummary: $("#removal-summary"), measureCapacityWarning: $("#measure-capacity-warning"), measureCapacityWarningText: $("#measure-capacity-warning-text"), measureCapacityWarningOpen: $("#measure-capacity-warning-open"),
  ...
  function positionSettingsPanel() {
    ...
    settingsPositionFrame = requestAnimationFrame(() => {
      if (settingsMode !== "closed") return;
      const currentClearance = Number.parseFloat(getComputedStyle(elements.workspace).getPropertyValue("--settings-clearance")) || 0;
      const baseResultTop = elements.outputShell.closest(".editor-card").getBoundingClientRect().top - currentClearance;
      const settingsBottom = elements.settingsPanel.getBoundingClientRect().bottom;
      const clearance = Math.max(0, Math.ceil(settingsBottom + 10 - baseResultTop));
      elements.workspace.style.setProperty("--settings-clearance", `${clearance}px`);
